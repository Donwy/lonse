...
public class MyPermissionUtils {
    ...
    public static boolean judgePermission(Context context, String permission) {
        ...
        String per = StringCache.getInstance().queryCache(permission,"");
        if (per != null){
            isFirst = per.equals("");
        }
        if (ActivityCompat.shouldShowRequestPermissionRationale((Activity) context, permission) || isFirst) {
            return true;
        } else {
            return false;
        }
    ...
    public static void onRequestPermissionResult(Context context, String permission, int[] grantResults, PermissionCheckCallBack callback) {
        if (MyPermissionUtils.isPermissionRequestSuccess(grantResults)) {
            ...
            if (MyPermissionUtils.judgePermission(context, permission)) {
                callback.onUserHasAlreadyTurnedDown(permission);
            } else {
                callback.onUserHasAlreadyTurnedDownAndDontAsk(permission);
            }
